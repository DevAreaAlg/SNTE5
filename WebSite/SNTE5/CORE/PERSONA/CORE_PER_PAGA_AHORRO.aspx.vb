...
Public Class CORE_PER_PAGA_AHORRO
    Inherits Page
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load

        TryCast(Master, MasterMascore).CargaASPX("Pago de ahorro por transferencia", "Pago de ahorro por Transferencia")
...
#Region "Carga Inicial"
    ...
    Private Sub LlenarExpXPagar(tipo As String, estatus As Integer, ciclo As Integer)
        ...
        Session("cmd").CommandText = "SEL_AHORRO_SPEI"
        Session("parm") = Session("cmd").CreateParameter("TIPO_TRAB", Session("adVarChar"), Session("adParamInput"), 1000, tipo)
        ...
        If DAG_Analisis.Rows.Count > 0 Then
            ...
            lbl_registros_tol.Text = "Total de Agremiados: " + DAG_Analisis.Rows.Count.ToString
            lbl_layout_bancos.Visible = True
            fud_layout_bancos.Visible = True
            btn_pagar.Visible = True
        Else
            btn_layout_bancos.Visible = False
            lbl_registros_tol.Text = "Total de Agremiados: 0"
    ...
    Private Sub DescargaTXTLayout()
        ...
        For i As Integer = 0 To DAG_Analisis.Rows.Count() - 1
            If Convert.ToInt32(DirectCast(DAG_Analisis.Rows(i).FindControl("chk_Aplicado"), CheckBox).Checked) = 1 Then
                dtBancos.Rows.Add(0,
                                  ...
                                  "",
                                  CDec(DAG_Analisis.Rows(i).Cells(5).Text))
            End If
        Next
        ...
        Try
            ...
            Using connection As New SqlConnection(ConfigurationManager.ConnectionStrings("ConnectionStringDotNet").ToString)
                ...
                Dim insertCommand As New SqlCommand("SEL_PAGO_A_BANCOS_AHORRO", connection)
                ...
                Session("parm") = New SqlParameter("TIPOTRAB", SqlDbType.VarChar)
                Session("parm").Value = tipo_trab.SelectedItem.ToString
                insertCommand.Parameters.Add(Session("parm"))
    ...
    Private Sub PagaCreditosPDs()
        ...
        Try
            ...
            Using connection As New SqlConnection(ConfigurationManager.ConnectionStrings("ConnectionStringDotNet").ToString)
                ...
                While myReader.Read()
                    If myReader.GetString(0) = "0PDs" Then
                        lbl_status.Text = "Error: No se selecciono ningun préstamo de ahorro deudor por pagar."
                    Else
...
#Region "Procesa Layout Bancos"
    ...
    Private Sub PagaCreditos()
        ...
        Try
            ...
            Session("cmd").Parameters.Append(Session("parm"))
            Session("parm") = Session("cmd").CreateParameter("CICLO", Session("adVarChar"), Session("adParamInput"), 20, cmb_ciclo.SelectedItem.Value.ToString)
            Session("cmd").Parameters.Append(Session("parm"))

            Session("cmd").CommandText = "INS_PROCESA_LAYOUT_BANCOS_AHORRO"
            Session("rs") = Session("cmd").Execute()

            lbl_status.Text = "Se ha procesado con éxito el layout de bancos."
...
#End Region
    ...
    Private Sub CargaTipoTrabajador()
        tipo_trab.Items.Clear()
        Try
            Session("cmd") = New ADODB.Command()
            Session("Con").Open()
            Session("cmd").ActiveConnection = Session("Con")
            Session("cmd").CommandType = System.Data.CommandType.StoredProcedure
            Session("cmd").CommandText = "SEL_TIPO_TRAB_AHORRO"
            Session("rs") = Session("cmd").Execute()

            Do While Not Session("rs").EOF
                tipo_trab.Items.Add(New ListItem(Session("rs").Fields("TIPO").Value, Session("rs").Fields("NUMERO").Value.ToString))

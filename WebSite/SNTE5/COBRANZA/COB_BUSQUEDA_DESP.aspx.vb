Public Class COB_BUSQUEDA_DESP
    ...
    Protected Sub Page_Prerrender(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRender
        ...
        End If
        TryCast(Me.Master, MasterMascore).CargaASPX("Búsqueda Despacho", "DESPACHOS")
    ...
    Private Sub btn_nuevo_Click(sender As Object, e As EventArgs) Handles btn_nuevo.Click
        Session("DESPACHOID_M") = "-1"
        Response.Redirect("COB_DESP_PERSONAM.aspx")
    End Sub
    Private Sub Elimina_Prospecto(ByVal despachoid As Integer)
        ...
        Session("cmd").Parameters.Append(Session("parm"))
        Session("cmd").CommandText = "UPD_COB_DESP_BAJA_PM"
        Session("cmd").Execute()
        Session("Con").Close()
    End Sub

    Private Sub dag_pendientes_ItemCommand(source As Object, e As DataGridCommandEventArgs) Handles dag_pendientes.ItemCommand
        Session("DESPACHOID_M") = CInt(e.Item.Cells(0).Text)

        If (e.CommandName = "EDITAR") Then
            Session("DESPACHOID_M") = e.Item.Cells(0).Text
            Response.Redirect("COB_DESP_PERSONAM.aspx")
        End If
        If (e.CommandName = "ELIMINAR") Then
            Elimina_Prospecto(Session("DESPACHOID_M"))
            Session("DESPACHOID_M") = Nothing
            LlenaPendientes()
        End If
        If (e.CommandName = "RESUMEN") Then
            Session("DESPACHOID_M") = e.Item.Cells(0).Text
            Response.Redirect("COB_DESP_RESUMEN_DESP.aspx")
        End If
    End Sub

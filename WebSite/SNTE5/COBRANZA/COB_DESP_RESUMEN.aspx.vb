Public Class COB_DESP_RESUMEN
    Inherits System.Web.UI.Page

    Protected Sub Page_Prerrender(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRender
        TryCast(Me.Master, MasterMascore).CargaASPX("Resumen De Abogado", "RESUMEN DE ABOGADO")
        If Not Me.IsPostBack Then
            resumenDir(Session("DESPACHOID"))
            resumenPers(Session("DESPACHOID"))
            resumenContacto(Session("DESPACHOID"))
            resumenUsuarios(Session("DESPACHOID"))
        End If
    ...
    Private Sub resumenPers(ByVal despachoid As Integer)
        ...
        If Not Session("rs").EOF Then
            ...
            If Session("rs").fields("RES").value.ToString = "1" Then
                ...
                lbl_nombre_res.Text = Session("rs").Fields("NOMBRE1").Value + " " + Session("rs").Fields("NOMBRE2").Value + " " + Session("rs").Fields("PATERNO").Value + " " + Session("rs").Fields("MATERNO").Value
                lbl_curp_res.Text = Session("rs").Fields("CURP").Value
    ...
    Private Sub resumenDir(ByVal despachoid As String)
        'carga_personales(despachoid)
        'carga_contacto(despachoid)
        'Cargausuarios(despachoid)
        ...
        Session("cmd").Parameters.Append(Session("parm"))
        Session("cmd").CommandText = "SEL_COB_DESP_DIRECCIONES_PF"
        Session("rs") = Session("cmd").Execute()
        If Not Session("rs").EOF Then

Imports System.IO
Public Class CRED_EXP_APARTADO9
    ...
    Public Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            ...
            Session("VENGODE") = "CRED_EXP_APARTADO9.aspx"
            ...
            If Session("ESTATUS_EXPEDIENTE") = "DOCUMENTOS POR DIGITALIZAR" Then
                ...
                btn_Insertar_ColaValidacion.Enabled = True
            Else
                MostrarDocumentosNoDigitalizadosFase2()
                btn_Eliminar.Enabled = False
                btn_Insertar_ColaValidacion.Enabled = False
            End If
            ...
            lbl_TamMax.Text = "Nota: El tamaño del documento no puede exceder los " + Session("TAMDIG") + " KB"
        End If
    ...
    Protected Sub lnk_Regresar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnk_regresar.Click
        ...
        EstatusEnDigitalizacion()
        Session("VENGODE") = "CRED_EXP_APARTADO9.aspx"
        Response.Redirect("CRED_EXP_GENERAL.aspx")
...
#End Region
    ...
    Protected Sub btn_Ver_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Ver.Click
        ...
        Else
            ...
            If RutaDigitaliza <> "" Then
                ...
                Else
                    lbl_AlertaVerBorrar.Text = "Error: El archivo ha sido movido o eliminado"
                End If
            Else
                lbl_AlertaVerBorrar.Text = "Error: No existe una ruta"
            End If
    ...
    Private Sub EliminarDocumentoDigitalizado() 'Metodo para eliminar el Documento Seleccionado
        'No permite eliminar un documento que ya halla sido aprobado por validadores
        Dim Nombre As String = ""
        Dim Ext As String = ""
        ...
        If Nombre <> "" And Ext <> "" Then
            ...
            If AUX = 0 Then
                lbl_AlertaNoBorrar.Text = "Error: El documento ya está aceptado por el validador, no puede ser borrado"
            Else
                If AUX = 1 Then
                    DelHDFile(cPath + archivo)
                Else
                    lbl_AlertaNoBorrar.Text = "Se ha eliminado la dependencia. Existe otro préstamo ocupando el mismo archivo."
                End If
            ...
            Session("Con").Close()
        Else
            lbl_UploadEstatus.Text = "No hay nombre"
        End If
    ...
    Protected Sub btn_Insertar_ColaValidacion_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Insertar_ColaValidacion.Click
        ...
        AvisoCambioEstatus()
        EnviaaMesaControlVAL()

        Session("VENGODE") = "CRED_EXP_APARTADO9.aspx"
        If Session("TIPOP") = "CAP" Then
            Response.Redirect("CAP_EXP_GENERAL.aspx")
        Else
            Response.Redirect("/CREDITO/EXPEDIENTES/CRED_EXP_GENERAL.aspx")
        End If
    ...
    Protected Sub btn_subir_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Subir.Click
        ...
        Else
            Try
                ...
                Dim nombrearchivo As String = Fecha + Hora + Left(Session("FOLIO"), 1) + value.ToString
                ...
                Else
                    ...
                    If size <= (Tamdig * 1024) Then
                        ...
                        FileUpload1.SaveAs(urldigi + "\" + nombrearchivo)
            ...
            Catch ex As Exception
                lbl_AlertaDigitaliza.Text = ex.ToString
            Finally
                MostrarDocumentosNoDigitalizados()
                MostrarDocumentosDigitalizados() 'Mostrara los documentos ya Dititalizados
                VerificarDocsCompletos() 'Verifica si ya estan digitalizados todos los requerimientos
    ...
    Private Sub EnviaaMesaControlVAL()
        ...
        Session("parm") = Session("cmd").CreateParameter("CLAVEEVENTO", Session("adVarChar"), Session("adParamInput"), 20, "EXPDPENVAL")
        Session("cmd").Parameters.Append(Session("parm"))
        Session("cmd").CommandText = "SEL_EMAIL_EVENTOS"
        Session("rs") = Session("cmd").Execute()

        Dim subject As String = "Expediente " + Session("CVEEXPE").ToString + " ha sido enviado a Mesa de Control para validar"
        ...
        Do While Not Session("rs").EOF
            ...
            sbhtml.Append("<br/>")
            sbhtml.Append("<tr><td>Se le informa que la documentación del expediente digital correspondiente al folio " + Session("CVEEXPE").ToString + "</td></tr>")
            sbhtml.Append("<br/>")
            sbhtml.Append("<tr><td>del agremiado " + Session("CLIENTE").ToString + " ha sido enviada a Mesa de Control para su validación.</td></tr>")

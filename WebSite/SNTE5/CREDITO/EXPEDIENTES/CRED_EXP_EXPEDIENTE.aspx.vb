Public Class CRED_EXP_EXPEDIENTE
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        End If
        'lnk_detalle_comite.Attributes.Add("OnClick", "det_com()")
        lnk_restructura.Attributes.Add("OnClick", "det_restructura()")
        'reviso que el usuario pertenezca al comite para habilitar tab de foro/votacion
    ...
    Private Sub PerteneceComite()
        ...
        Session("cmd").Parameters.Append(Session("parm"))
        Session("cmd").CommandText = "SEL_ANAEXP_PERTENECE_COMITE_CREDITO"
        Session("rs") = Session("cmd").Execute()

        Dim pertenece As String = Session("rs").Fields("PERTENECE").value.ToString()
        Dim dictamen As String = Session("rs").Fields("DICTAMEN").value.ToString()
        Session("Con").Close()

        If pertenece = "SI" Then
            'muestro los comentarios del foro con respecto al expediente
            MuestraComentarios()
            Dim res As String = YaVoto()
            Votacion()
            'habilito el panel de foro/votacion
    ...
    Private Function YaVoto() As String
        ...
        Session("cmd").ActiveConnection = Session("Con")
        Session("cmd").CommandType = System.Data.CommandType.StoredProcedure
        Session("parm") = Session("cmd").CreateParameter("IDSESCOMIT", Session("adVarChar"), Session("adParamInput"), 10, Session("IDSESCOMIT"))
        Session("cmd").Parameters.Append(Session("parm"))
        Session("parm") = Session("cmd").CreateParameter("IDUSER", Session("adVarChar"), Session("adParamInput"), 10, Session("USERID"))
        Session("cmd").Parameters.Append(Session("parm"))
        Session("cmd").CommandText = "SEL_ANAEXP_YA_VOTO"
        Session("rs") = Session("cmd").Execute()
    ...
    Private Sub Votacion()
        ...
        Session("parm") = Session("cmd").CreateParameter("IDSESCOMIT", Session("adVarChar"), Session("adParamInput"), 10, Session("IDSESCOMIT"))
        Session("cmd").Parameters.Append(Session("parm"))
        Session("cmd").CommandText = "SEL_ANAEXP_VOTACION_SESION"
        Session("rs") = Session("cmd").Execute()
        lbl_votosf.Text = "Votos a favor: " + Session("rs").Fields("FAVOR").value.ToString()
        lbl_votosc.Text = "Votos en contra: " + Session("rs").Fields("CONTRA").value.ToString()
        lbl_votosa.Text = "Votos en abstención: " + Session("rs").Fields("ABSTENCION").value.ToString()
    ...
    Protected Sub lnk_garantias_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnk_garantias.Click
        Session("VENGODE") = "CRED_EXP_EXPEDIENTE.ASPX"
    ...
    Private Sub DetalleExpediente()
        ...
        End If
        Session("Con").Close()
        If comite = "1" Then 'ya fue enviado a analisis directo a comite
    ...
    Private Sub MuestraComentarios()
        ...
        Session("cmd").ActiveConnection = Session("Con")
        Session("cmd").CommandType = System.Data.CommandType.StoredProcedure
        Session("parm") = Session("cmd").CreateParameter("IDSESFORO", Session("adVarChar"), Session("adParamInput"), 10, Session("IDSESCOMIT"))
        Session("cmd").Parameters.Append(Session("parm"))
        Session("cmd").CommandText = "SEL_ANAEXP_COMENTARIOS_FORO"
        Session("rs") = Session("cmd").Execute()
    ...
    Protected Sub btn_enviar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_enviar.Click
        ...
        Session("cmd").ActiveConnection = Session("Con")
        Session("cmd").CommandType = System.Data.CommandType.StoredProcedure
        Session("parm") = Session("cmd").CreateParameter("IDSESFORO", Session("adVarChar"), Session("adParamInput"), 15, Session("IDSESCOMIT"))
        Session("cmd").Parameters.Append(Session("parm"))
        ...
        Session("cmd").CommandText = "INS_ANAEXP_COMENTARIO_FORO"
        Session("cmd").Execute()
    ...
    Protected Sub btn_votar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_votar.Click
        ...
        If res = "NO" Then
            ...
            Session("cmd").CommandText = "INS_ANAEXP_VOTO_COMITE"
            Session("cmd").Execute()

            Session("Con").Close()

            lbl_status.Text = "Voto emitido correctamente."
            cmb_voto.Enabled = False
            btn_votar.Enabled = False
    ...
    Protected Sub btn_resultado_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_resultado.Click
        If cmb_resultado.SelectedItem.Value = "RECHAZADO" Or cmb_resultado.SelectedItem.Value = "RECHAZOREC" Then
            Dim ruta As String
        ...
        End If
        Session("parm") = Session("cmd").CreateParameter("MONTO_AUTOR", Session("adVarChar"), Session("adParamInput"), 15, txt_MontoAutor.Text)
        Session("cmd").Parameters.Append(Session("parm"))
        Session("parm") = Session("cmd").CreateParameter("RECOMENDACIONES", Session("adVarChar"), Session("adParamInput"), 1000, txt_Recomendaciones.Text)
        Session("cmd").Parameters.Append(Session("parm"))
        ...
        lbl_resdictamen.Text = "Guardado correctamente. Favor de prellenar, firmar y digitalizar"
    ...
    Protected Sub lnk_dictamen_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnk_dictamen.Click
        ...
        'Session("VENGODE") = "ANAEXP_EXPEDIENTE.ASPX"
        ...
        With Response
            ...
            .AddHeader("Content-disposition",
                       "attachment; filename= Dictamen.pdf")
    ...
    Private Sub PrellenadoActaDictamen()
        ...
        Reader = New iTextSharp.text.pdf.PdfReader(Session("APPATH").ToString + "\DocPlantillas\ActaComCredito.pdf")
        ...
        With document
            .AddAuthor("PENSIONES")
            .AddCreationDate()
            .AddCreator("Dictamen Comite")
            .AddSubject("Dictamen Comite - " + Session("FOLIO"))
            'Use the filename as the title... You can give it any title of course.        
            .AddTitle("Dictamen Comite - " + Session("FOLIO"))
            'Add keywords, whatever keywords you want to attach to it       
            .AddKeywords("Dictamen Comite - " + Session("FOLIO"))
            .Open()
        End With
        ...
        Session("cmd").ActiveConnection = Session("Con")
        Session("cmd").CommandType = System.Data.CommandType.StoredProcedure
        Session("cmd").CommandText = "SEL_SUCURSAL_CREDITO"
        ...
        If Not Session("rs").eof Then
            ...
            cb.ShowTextAligned(iTextSharp.text.pdf.PdfContentByte.ALIGN_LEFT, Session("rs").Fields("SUCURSAL").Value.ToString, X, Y, 0)
        ...
        If Not Session("rs").eof Then
            ...
            End If
            cb.ShowTextAligned(iTextSharp.text.pdf.PdfContentByte.ALIGN_LEFT, resultado + " (" + Session("rs").Fields("USR_RESULTADO").Value.ToString + ")", X, Y, 0)
        ...
        If Not Session("rs").eof Then
            ...
            Do While Not Session("rs").EOF
                ...
                If (Y - 50) < 30 Then
                    ...
                    document.NewPage()
                    Reader = New iTextSharp.text.pdf.PdfReader(Session("APPATH").ToString + "\Archivos\ActaComCreditoBlanco.pdf")
    ...
    Protected Sub lnk_actualiza_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnk_actualiza.Click
        MuestraComentarios()
        lbl_status.Text = ""
    End Sub
    ...
    Protected Sub lnk_restructura_Click(sender As Object, e As EventArgs) Handles lnk_restructura.Click
